...
class RestExceptionHandler {
    ...
    fun handlerValidException(ex: MethodArgumentNotValidException): ResponseEntity<ExceptionDetails> {
        ...
        return ResponseEntity(
            ExceptionDetails(
                ...
                exception = "[${ex.javaClass}], Object: ${ex.objectName}",
                details = erros
    ...
    fun handlerValidException(ex: DataAccessException): ResponseEntity<ExceptionDetails> {
        return ResponseEntity.status(HttpStatus.CONFLICT)
            .body(
                ExceptionDetails(
                    title = "Bad Conflit! Consult the documentation",
                    timestamp = LocalDateTime.now(),
                    status = HttpStatus.CONFLICT.value(),
                    exception = "[${ex.javaClass}]",
    ...
    fun handlerValidException(ex: IllegalArgumentException): ResponseEntity<ExceptionDetails> {
        return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(
            ExceptionDetails(
                title = "Bad Request! Contact the documentation",
                timestamp = LocalDateTime.now(),
                status = HttpStatus.BAD_REQUEST.value(),
                exception = "${ex.javaClass}",
                details = mutableMapOf(ex.cause.toString() to ex.message)
